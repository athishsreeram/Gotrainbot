...
ALIASES = {
    "ki": "kitchener", "kit": "kitchener",
    "guelph": "guelphcentral", "gl": "guelphcentral", "gue": "guelphcentral",
    "ac": "acton",
    "geo": "georgetown", "ge": "georgetown",
    "mp": "mountpleasant", "mo": "mountpleasant", "mount": "mountpleasant", "pleasant": "mountpleasant",
    "br": "brampton", "bra": "brampton", "bramptoninnovation": "brampton",
    "be": "bramalea", "bram": "bramalea", "bml": "bramalea",
    "ma": "malton", "mal": "malton",
    "we": "weston",
    "md": "mountdennis", "dennis": "mountdennis",
    "bl": "bloor",
    "un": "union", "unionstation": "union",
}
...
def fetch_departures(station_code: str, direction: str) -> list[dict]:
    ...
    try:
        ...
        text = resp.content.decode("utf-8-sig")  # <-- handles BOM
        import json
        data = json.loads(text)
    except Exception as e:
        logger.exception("API fetch/parse failed")
        return []
    ...
    for dir_block in data.get("directions", []):
        ...
        for trip in dir_block.get("tripMessages", []):
            scheduled_dt = _parse_dt(trip.get("scheduled", ""))
            actual_dt    = _parse_dt(trip.get("actual", ""))

            delay_str = ""
            if scheduled_dt and actual_dt:
                delta = (actual_dt - scheduled_dt).total_seconds()
                if delta > 60:
                    delay_str = f"+{int(delta // 60)} min late"
                elif delta < -60:
                    delay_str = f"{int(delta // 60)} min early"
                else:
                    delay_str = "On time"

            sched_str = scheduled_dt.strftime("%-I:%M %p") if scheduled_dt else trip.get("scheduled", "")
            actual_str = actual_dt.strftime("%-I:%M %p")   if actual_dt    else trip.get("actual", "")
            stops = [s.get("stopName") for s in trip.get("stopsList", []) if s.get("stopName")]
...
async def _handle(update: Update, context: ContextTypes.DEFAULT_TYPE, direction: str):
    ...
    if not code:
        await update.message.reply_text(
            f"❓ Unknown station: *{raw_input}*\n\nSend /stations for the station list.",
            parse_mode="Markdown"
        )
        return

    arrow = "→ Union 🏙️" if direction == "from" else "← from Union 🏙️"
    await update.message.reply_text(f"⏳ Fetching *{display}* {arrow}", parse_mode="Markdown")

    try:
        loop = asyncio.get_event_loop()
        trips = await loop.run_in_executor(executor, fetch_departures, code, direction)
    except Exception as e:
        logger.exception("Fetch failed")
        await update.message.reply_text(f"⚠️ Error fetching data: {e}")
        return

    title = f"{display} → Union" if direction == "from" else f"Union → {display}"
